...
public class SpaceDAO implements ISpaceDAO {
    ...
    public Space saveSpace(Space space) {
        if (space.getSpaceId() == null) {
            // 插入新記錄
            String sql = "INSERT INTO Space (parkingId) VALUES (?)";
            jdbcTemplate.update(sql, space.getParkingId());
            ...
            // 更新現有記錄
            String sql = "UPDATE Space SET parkingId = ? WHERE spaceId = ?";
            jdbcTemplate.update(sql, space.getParkingId(), space.getSpaceId());
            return space;
        }
    }
